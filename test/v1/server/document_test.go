...
func (s *DocumentSuite) TestInsert_AutoGenerated() {
	collectionName := "test_autogenerated"
	s.testAutoGenerated(Map{
		"schema": Map{
			"title": collectionName,
			"properties": Map{
				"int_value": Map{
...
func (s *DocumentSuite) testAutoGenerated(schema Map, collectionName string, generatorType string) {
	dropCollection(s.T(), s.database, collectionName)
	createCollection(s.T(), s.database, collectionName, schema).Status(http.StatusOK)
	...
	readResp := readByFilter(s.T(), s.database, collectionName, nil, nil)
	var decodedResult = make([]map[string]interface{}, 3)
	for _, response := range readResp {
		var doc map[string]json.RawMessage
		require.NoError(s.T(), json.Unmarshal(response["result"], &doc))
		var actualDoc map[string]interface{}
		require.NoError(s.T(), json.Unmarshal(doc["data"], &actualDoc))

		val := int64(actualDoc["int_value"].(float64))
		if val == 1 {
			decodedResult[0] = actualDoc
		} else if val == 2 {
			decodedResult[1] = actualDoc
		} else if val == 3 {
			decodedResult[2] = actualDoc
		} else {
			require.Fail(s.T(), fmt.Sprintf("not expected value %d %T", val, actualDoc["int_value"]))
		}
	}

	validate := func(response map[string]interface{}, inputDoc Doc, iteration int) {
		require.Equal(s.T(), inputDoc["int_value"], int(response["int_value"].(float64)))
		require.NotNil(s.T(), response["pkey"])
		switch generatorType {
		case "int32":
			require.Equal(s.T(), int(response["pkey"].(float64)), iteration+1)
		case "string", "uuid":
			_, err := uuid.Parse(response["pkey"].(string))
			require.NoError(s.T(), err)
		case "int64":
			ts := time.Unix(0, int64(response["pkey"].(float64)))
			ts.After(time.Now().Add(-1 * time.Minute))
			require.True(s.T(), ts.After(time.Now().Add(-1*time.Minute)))
		case "datetime":
