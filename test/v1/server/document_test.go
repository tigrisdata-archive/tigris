...
func TestInsert_AutoGenerated(t *testing.T) {
	dbName := fmt.Sprintf("db_test_autogenerated_%x", rand.Uint64())
	...
	testAutoGenerated(t, dbName, collectionName, Map{"type": "string", "autoGenerate": true})
	testAutoGenerated(t, dbName, collectionName, Map{
		"type":         "string",
		"autoGenerate": true,
		"format":       "byte"})
	testAutoGenerated(t, dbName, collectionName, Map{
	...
	testAutoGenerated(t, dbName, collectionName, Map{"type": "integer", "autoGenerate": true})
}
...
func testAutoGenerated(t *testing.T, dbName string, collectionName string, pkey Map) {
	...
	validate := func(response map[string]interface{}, inputDoc Doc, iteration int) {
		...
		case "string", "uuid":
			...
			require.NotEqual(t, uuid.UUID{}.String(), s)
		case "int64":
			if iteration == 4 {
				require.Equal(t, 123, int(response["pkey"].(float64)))
			} else {
