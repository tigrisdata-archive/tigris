...
func (q *SecondaryIndexer) scanIndex(ctx context.Context, tx transaction.Tx, coll *schema.DefaultCollection, start keys.Key, end keys.Key) (*IndexIterator, error) {
	it, err := tx.ReadRange(ctx, start, end, false)
	if err != nil {
		return nil, err
	}
...
func (q *SecondaryIndexer) Update(ctx context.Context, tx transaction.Tx, newTd *internal.TableData, oldTd *internal.TableData, primaryKey []interface{}) error {
	...
	// Update Row Count
	incAtomicMap(ctx, tx, q.coll, genRowCountKey, updateSet.addCounts)
	decAtomicMap(ctx, tx, q.coll, genRowCountKey, updateSet.removeCounts)

	// Update Row Size
	incAtomicMap(ctx, tx, q.coll, genRowSizeKey, updateSet.addSizes)
	decAtomicMap(ctx, tx, q.coll, genRowSizeKey, updateSet.removeSizes)
...
func (q *SecondaryIndexer) buildAddAndRemoveKVs(newTableData *internal.TableData, oldTableData *internal.TableData, primaryKey []interface{}) *IndexerUpdateSet {
	newRows := q.buildTableRows(newTableData, primaryKey)
	oldRows := q.buildTableRows(oldTableData, primaryKey)
...
func (q *SecondaryIndexer) buildTableRows(tableData *internal.TableData, primaryKey []interface{}) []IndexRow {
	if tableData == nil {
		return []IndexRow{}
	}
	rows := q.buildTSRows(tableData, primaryKey)
...
func (q *SecondaryIndexer) buildTSRows(tableData *internal.TableData, primaryKey []interface{}) []IndexRow {
	var rows []IndexRow
...
func (q *SecondaryIndexer) createKeysAndIndexInfo(primaryKey []interface{}, rows []IndexRow) ([]keys.Key, map[string]int64, map[string]int64) {
	...
	for _, row := range rows {
		...
		if row.stub == true && stubs[row.Name()] == true {
			continue
		} else {
			stubs[row.Name()] = true
		}
...
func (q *SecondaryIndexer) indexNestedField(doc []byte, topField string, pos int) []IndexRow {
	...
	jsonparser.ObjectEach(doc, processor)
...
func (q *SecondaryIndexer) indexArray(doc []byte, field *schema.QueryableField, keyPath []string) []IndexRow {
	...
	jsonparser.ArrayEach(doc, processor, keyPath...)
	return rows
}
...
func getFieldVersion(fieldName string, coll *schema.DefaultCollection) int {
	...
	if fieldVersions != nil && len(fieldVersions) > 0 {
