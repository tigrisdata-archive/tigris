...
func (runner *BaseQueryRunner) insertOrReplace(ctx context.Context, tx transaction.Tx, tenant *metadata.Tenant,
	...
	queryIndexer := NewSecondaryIndexer(coll)
	for _, doc := range documents {
		...
		// Add document to secondary index
		queryIndexer.Index(ctx, tx, tableData, key.IndexParts())
...
func (runner *BaseQueryRunner) buildKeysUsingFilter(coll *schema.DefaultCollection,
	...
	kb := filter.NewKeyBuilder(
		filter.NewStrictEqKeyComposer(func(indexParts ...interface{}) (keys.Key, error) {
			return runner.encoder.EncodeKey(coll.EncodedName, primaryKeyIndex, indexParts)
		}, filter.PKBuildIndexPartsFunc))

	pkKeys, err := kb.Build(filters, coll.Indexes.PrimaryKey.FieldsToQueryable())
	if err != nil {
		return nil, err
	}

	if len(pkKeys) == 0 {
		return nil, errors.InvalidArgument("Could not find a primary key to query on")
	}

	return pkKeys[0].Keys, nil
}
...
func (runner *StreamingQueryRunner) buildReaderOptions(collection *schema.DefaultCollection) (readerOptions, error) {
	...
	if options.ikeys, err = runner.buildSecondaryIndexKeysUsingFilter(collection, runner.req.Filter, collation); err == nil {
		options.queryIndex = true
		return options, nil
	}
	...
	if options.ikeys, err = runner.buildKeysUsingFilter(collection, runner.req.Filter, collation); err == nil {
		return options, nil
	}
...
func (runner *StreamingQueryRunner) instrumentRunner(ctx context.Context, options readerOptions) context.Context {
	...
	if options.queryIndex {
		runner.queryMetrics.SetReadType("query-index")
	}
...
func (runner *StreamingQueryRunner) Run(ctx context.Context, tx transaction.Tx, tenant *metadata.Tenant) (Response, context.Context, error) {
	...
	if options.inMemoryStore {
		err = runner.iterateOnSearchStore(ctx, coll, options)
	} else if options.queryIndex {
		_, err = runner.iterateOnSecondaryIndexStore(ctx, tx, coll, options)
	} else {
		_, err = runner.iterateOnKvStore(ctx, tx, coll, options)
	}
...
func (runner *StreamingQueryRunner) iterateOnSecondaryIndexStore(ctx context.Context, tx transaction.Tx, coll *schema.DefaultCollection, options readerOptions) ([]byte, error) {
	reader := NewSecondaryIndexReader(ctx, tx, coll, options.filter, options.ikeys)

	iter, err := reader.CreateIter()
	if err != nil {
		return nil, err
	}
