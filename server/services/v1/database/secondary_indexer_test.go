...
func TestIndexingStoreAndGetSimpleKVsforDoc(t *testing.T) {
	...
	kvStore.DropTable(ctx, []byte("t1"))
	kvStore.CreateTable(ctx, []byte("t1"))
	indexStore := setupTest(t, reqSchema)
	...
	t.Run("insert", func(t *testing.T) {
		...
		start := keys.NewKey(coll.EncodedName, coll.Indexes.SecondaryIndex.Name, KVSubspace)
		end := keys.NewKey(coll.EncodedName, coll.Indexes.SecondaryIndex.Name, KVSubspace, 0xFF)
		iter, err := indexStore.scanIndex(ctx, tx, coll, start, end)
		assert.NoError(t, err)
		...
		assert.NoError(t, err)
		assert.NoError(t, iter.Interrupted())
	...
	t.Run("delete", func(t *testing.T) {
		...
		for iter.Next(&row) {
			...
			//Make sure all rows returned are id2 because id1 doc is removed
