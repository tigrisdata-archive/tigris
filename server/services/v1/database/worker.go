...
import (
	"context"
	"github.com/tigrisdata/tigris/server/services/v1/database/worker"
	"time"
...
type schemaDocumentJob struct {
}

func (s *schemaDocumentJob) StartCollWork(tx *transaction.Tx, coll *schema.DefaultCollection) bool {
	return false
}
func (s *schemaDocumentJob) FinishCollWork(tx *transaction.Tx, coll *schema.DefaultCollection) error {
	return nil
}

func (s *schemaDocumentJob) ProcessRow(tx *transaction.Tx, row map[string]any) error {
	return nil
}

func RepairSchemaLoop(tenantMgr *metadata.TenantManager) {
	for {
		j := worker.DocumentJob()
