...
func TestApplySchemaRules(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			errors.InvalidArgument("removing a field is a backward incompatible change. missing: s"),
		},
		{
			...
			errors.InvalidArgument("removing a field is a backward incompatible change. missing: s.nested2"),
		},
		{
			...
			errors.InvalidArgument("data type mismatch for field \"s.nested2\""),
		},
		{
			"primary key added",
			[]byte(`{ "title": "t1", "properties": { "id": { "type": "integer"}, "s": { "type": "string"}},"primary_key": ["id"]}`),
		...
		{
			"object_to_map_explicit",
			[]byte(`{ "title": "t1", "properties": { "id": { "type": "integer" },
			"simple_object": { "type": "object", "properties" : { "one" : { "type" : "string" } } } },
		    "primary_key": ["id"]}`),
			[]byte(`{ "title": "t1", "properties": { "id": { "type": "integer" },
			"simple_object": { "type": "object", "additionalProperties" : true } },
			"primary_key": ["id"]}`),
			nil,
		},
		{
			"object_to_map_implicit",
			[]byte(`{ "title": "t1", "properties": { "id": { "type": "integer" },
			"simple_object": { "type": "object", "properties" : { "one" : { "type" : "string" } } } },
		    "primary_key": ["id"] }`),
			[]byte(`{ "title": "t1", "properties": { "id": { "type": "integer" },
			"simple_object": { "type": "object" } },
		    "primary_key": ["id"] }`),
			errors.InvalidArgument("removing a field is a backward incompatible change. missing: simple_object.one"),
		},
	}
