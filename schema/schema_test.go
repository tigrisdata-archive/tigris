...
func TestCreateCollectionFromSchema(t *testing.T) {
	t.Run("test_create_success", func(t *testing.T) {
		...
		c := NewDefaultCollection("t1", 1, 1, schF.CollectionType, schF.Fields, schF.Indexes, schF.Schema, "t1")
	...
	"primary_key": ["id"]
		...
		sch, err := Build("t1", schema)
		require.NoError(t, err)
		coll := NewDefaultCollection("t1", 1, 1, sch.CollectionType, sch.Fields, sch.Indexes, sch.Schema, "t1")
