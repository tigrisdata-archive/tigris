...
func TestCreateCollectionFromSchema(t *testing.T) {
	t.Run("test_create_success", func(t *testing.T) {
		...
		c := NewDefaultCollection("t1", 1, 1, schF.CollectionType, schF, nil, nil)
	...
	"primary_key": ["K1", "K2"]
		...
		require.NoError(t, err)
		c := NewDefaultCollection("t1", 1, 1, sch.CollectionType, sch, nil, nil)
		fields := c.GetFields()
	...
	"primary_key": ["id"]
		...
		coll := NewDefaultCollection("t1", 1, 1, sch.CollectionType, sch, nil, nil)
		coll.Validator = nil
