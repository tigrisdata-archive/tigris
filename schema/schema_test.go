...
func TestCreateCollectionFromSchema(t *testing.T) {
	t.Run("test_create_success", func(t *testing.T) {
		...
		c := NewDefaultCollection("t1", 1, schF.Fields, schF.Indexes, schF.Schema, "t1")
	...
	"primary_key": ["K1", "K2"]
		...
		require.NoError(t, err)
		c := NewDefaultCollection("t1", 1, sch.Fields, sch.Indexes, sch.Schema, "t1")
		fields := c.GetFields()
	...
	"primary_key": ["id"]
		...
		sch, err := Build("t1", schema)
		require.NoError(t, err)
		coll := NewDefaultCollection("t1", 1, sch.Fields, sch.Indexes, sch.Schema, "t1")
	...
	t.Run("test_no-primary-key", func(t *testing.T) {
		schema := []byte(`{
	"title": "t1",
	"properties": {
		"K1": {
